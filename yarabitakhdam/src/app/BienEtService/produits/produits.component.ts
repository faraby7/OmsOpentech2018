...
export class ProduitsComponent {
    ...
    onSubmit() {
        ...
        if (this.myform.valid) {
            this.addToast({title:'Votre Produit '+this.myform.value['NomProduit']+' est en cours', msg:'', timeout:1000, theme:'bootstrap', position:'top-right', type:'wait'});
            ...
            this.produitPost.type = this.myform.value['Type'];
             ...
             this.produitService.addProduct( this.produitPost ).subscribe(produitPostconsole=>{
                try {
                    if(typeof produitPostconsole.RefProduit!='undefined'){
                        this.addToast({title:'Votre Produit '+this.myform.value['NomProduit']+' est ajouté à votre système', msg:'', timeout: 6000, theme:'bootstrap', position:'top-right', type:'success'});
                        this.myform.reset();
                        setTimeout(() => {this.ngOnInit();}, 1000);
                    }
                }catch (e) {
                    this.addToast({title:'Votre Produit'+this.myform.value['NomProduit']+' n\'est pas ajouté à votre système', msg:'error "Something went wrong on the server"', timeout: 12000, theme:'bootstrap', position:'top-right', type:'error'});
                } }
        );}
    }

    onUpdate(id,EtatVente,EtatAchat,TypeProduit,RefProduit,NomProduit,DescriptionProduit,DescriptionFacture,PrixVente,PrixVenteMin,TauxTVA,Type){

       if (this.myformUpdate.valid) {
           this.addToast({title:'Votre Produit '+NomProduit+' est en cours', msg:'', timeout:1000, theme:'bootstrap', position:'top-right', type:'wait'});
            this.produitPost=  new Produits();
            ...
            this.produitPost.type=this.myformUpdate.value['Type'] ? this.myformUpdate.value['Type']:Type;
           ...
           this.produitService.updateProduit( this.produitPost ).subscribe(produitPostconsole=>{
                   try {
                       ...
                       if(typeof produitPostconsole.RefProduit=='undefined'){
                           this.addToast({title:'Votre Produit '+NomProduit+' est modifié à votre système', msg:'', timeout: 6000, theme:'bootstrap', position:'top-right', type:'success'});
                           this.myform.reset();
                           setTimeout(() => {this.ngOnInit();}, 1000);
                       }
                   }catch (e) {
    ...
    delete(produit:Produits):void{
        ...
        this.produitService.deleteProduit(produit).subscribe(produitPostconsole=>{
            try {
                this.addToast({title:'Votre Produit est en cours', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'wait'});
                if(typeof produitPostconsole.RefProduit =='undefined'){
                    this.addToast({title:'Votre Produit est supprimé ', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'success'});
                    this.myform.reset();
                    setTimeout(() => {this.ngOnInit();}, 1000);
                }
            }catch (e) {
                ...
                this.produitService.UpdateEtat(produit).subscribe(produitPostconsole=>{
                    try {
                        if(typeof produitPostconsole.RefProduit =='undefined'){
                            this.addToast({title:'Votre Produit est Hore Vente Hors Achat ', msg:'', timeout: 2000, theme:'bootstrap', position:'top-right', type:'error'});
                            this.myform.reset();
                            setTimeout(() => {this.ngOnInit();}, 1000);
                        }
                    }catch (e) {
