...
@Component({
  ...
  templateUrl: './reglement-provider.component.html',
  styleUrls: ['./reglement-provider.component.scss']
})
export class ReglementProviderComponent {
    ...
    constructor(private reglementProviderService: ReglementProviderService, private http: HttpClient, private modalService: NgbModal, private toastyService: ToastyService) {
    ...
    getFactureProvider():void{
        this.reglementProviderService.getReglements().subscribe(facture => this.reglementData=facture);
    }
    ...
    onUpdate(id,RefFacture,MontantHT,MontantTVA,MontantTTC,NotePublic,NotePriver,EtatFacture ,DateEcheance,DateFacture,MontantReglement,MovementID,ProviderID){
        ...
        if (this.myformUpdate.valid) {
            ...
            this.reglementProviderService.updateFacture( this.facturePost ).subscribe(facturePostconsole=>{
                    try {
    ...
    delete(facture:Factures):void{
        ...
        this.reglementProviderService.deleteFacture(facture).subscribe(produitPostconsole=>{
    ...
    onSubmitLigne(id:number) {
        ...
        if (this.myformligne.valid) {
            ...
            this.ligneFacturePost.TVA = this.myformligne.value['TVA'];
            this.ligneFacturePost.Qte = this.myformligne.value['Qte'];
            this.ligneFacturePost.PUHT = this.myformligne.value['PUHT'];
            this.ligneFacturePost.Reducation = this.myformligne.value['Reducation'];
            ...
            //now
            this.reglementProviderService.addLignesFacture( this.ligneFacturePost ).subscribe(factureLignePostconsole=>{
                    try {
                        if(typeof factureLignePostconsole.TVA!='undefined'){
                            this.reglementProviderService.calculefacture(id).subscribe(fact=>{});
     ...
     getLigneFacture(id:number):void{
        this.reglementProviderService.getLignesFacture(id).subscribe(lignefacture => this.ligneFactureData=lignefacture);
    }

    getAllProduit():void{
        this.reglementProviderService.getallProduit().subscribe(produits => this.allProduit=produits);
    }

    getAllProvider():void{
        this.reglementProviderService.getallProvider().subscribe(providers => this.allProvider=providers);
    }
